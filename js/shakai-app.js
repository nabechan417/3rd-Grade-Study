...
const ShakaiApp = (() => {
  ...
  // 3年生社会の unit_id は 6001〜6007
  const UNIT_OFFSET = 6000;
  ...
  function renderHome() {
    const stars = _progress
      .filter(p => p.unit_id >= UNIT_OFFSET + 1 && p.unit_id <= UNIT_OFFSET + 7)
    ...
    const UNITS = [
      { id: 1, name: 'まちのようすをしらべよう',      icon: '🗺️', color: 'green',  hasContent: true  },
      { id: 2, name: '農家のしごと',                  icon: '🌾', color: 'orange', hasContent: false },
      { id: 3, name: '工場のしごと',                  icon: '🏭', color: 'mint',   hasContent: false },
      { id: 4, name: 'お店のしごと（スーパー）',      icon: '🛒', color: 'purple', hasContent: false },
      { id: 5, name: '火事をふせぐ',                  icon: '🚒', color: 'orange', hasContent: false },
      { id: 6, name: '事故・事件をふせぐ',            icon: '🚓', color: 'mint',   hasContent: false },
      { id: 7, name: '市のうつりかわり',              icon: '🏙️', color: 'purple', hasContent: false },
  ...
  function openUnit(unitId) {
    if (unitId === 1) ShakaiUnit1.resetLesson();

    showPage('unit');

    document.querySelector('.unit-header-title').textContent =
      unitId === 1 ? '単元1：まちのようすをしらべよう' : `単元${unitId}`;
    document.querySelector('.unit-header-sub').textContent = '東京書籍「新しい社会3」';
    ...
    if (unitId === 1) {
      ShakaiUnit1.renderExplanation();
      ShakaiUnit1.startPractice(async ({ score, maxScore, stars }) => {
        if (_user) {
          await Progress.save({
      ...
      ShakaiUnit1.startTest(async ({ score, maxScore, stars }) => {
        if (_user) {
          await Progress.save({
            userId: _user.id, unitId: UNIT_OFFSET + 1,
...
document.addEventListener('DOMContentLoaded', () => ShakaiApp.init());
